...
// Update routes incoming messages through the state-machine dispatch,
// delegating to one update* method per message family: terminal chrome
// (app_chrome.go), session/auth (app_session.go), room lifecycle
// (app_rooms.go), and chat/backfill (app_chat.go). Every behavior
// transition lives in those methods — sub-Models surface outcomes via
// typed messages that each family maps to the next state. The families
// claim disjoint message types, so probe order cannot change behavior;
// a message no family claims falls through to the open modal, if any.
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	if next, cmd, handled := m.updateChrome(msg); handled {
		return next, cmd
	}
	if next, cmd, handled := m.updateSession(msg); handled {
		return next, cmd
	}
	if next, cmd, handled := m.updateRooms(msg); handled {
		return next, cmd
	}
	if next, cmd, handled := m.updateChat(msg); handled {
		return next, cmd
	}
...
func (m Model) handleSessionExpiry() (Model, tea.Cmd) {
