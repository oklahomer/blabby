...
func TestUserGrainFor_UsesTestSeamWhenSet(t *testing.T) {
	...
	g := &Gateway{userGrain: func(userID id.UserID) userGrainCaller {
		if userID.String() != "1" {
			t.Fatalf("userGrainFor passed unexpected userID: got %q, want %q", userID.String(), "1")
		}
		return fake
	}}

	got := g.userGrainFor(mustUserID(t, "1"))
	if got != fake {
		t.Fatalf("userGrainFor: got %v, want injected fake %v", got, fake)
	}

	if _, err := got.JoinRoom(&userpb.JoinRoomRequest{RoomId: "4"}); err == nil {
