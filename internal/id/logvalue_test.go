...
// slog.JSONHandler that the production binary uses and asserts the rendered
...
func TestLogValuer_JSONHandler(t *testing.T) {
	...
	if got, want := line["user_id"], "1"; got != want {
		t.Errorf("user_id rendered as %v (%T), want %q", got, got, want)
	}
	if got, want := line["room_id"], "4"; got != want {
		t.Errorf("room_id rendered as %v (%T), want %q", got, got, want)
	}
}

// TestLogValue_ReturnsStringValue confirms the slog.Value kind, so a future
...
func TestLogValue_ReturnsStringValue(t *testing.T) {
	...
	if got := uid.LogValue().String(); got != "1" {
		t.Errorf("UserID.LogValue().String() = %q, want %q", got, "1")
	}
	if got := rid.LogValue().Kind(); got != slog.KindString {
		t.Errorf("RoomID.LogValue().Kind() = %v, want slog.KindString", got)
	}
	if got := rid.LogValue().String(); got != "4" {
		t.Errorf("RoomID.LogValue().String() = %q, want %q", got, "4")
	}
}
