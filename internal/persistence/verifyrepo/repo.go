...
var ErrVerificationNotFound = errors.New("verifyrepo: verification not found")
...
var ErrVerificationRateLimited = errors.New("verifyrepo: verification resend rate limited")
...
// Repo reads and writes the email_verification table. Its methods take a
...
// New returns a Repo. It is stateless — email_verification rows are keyed by an
// already-minted user_id, so the repo mints nothing.
func New() *Repo { return &Repo{} }

// CreateParams carries the fields of a fresh verification challenge. PinHash is the
...
func (r *Repo) Create(ctx context.Context, q postgres.Querier, params CreateParams) error {
	_, err := q.Exec(ctx, createSQL,
		params.UserID.Int64(), params.PinHash, params.ExpiresAt, params.SentAt)
	if err != nil {
		return fmt.Errorf("verifyrepo: create: %w", err)
	}
	return nil
}
...
func (r *Repo) FindByUser(ctx context.Context, q postgres.Querier, userID id.UserID) (Verification, error) {
	v, err := scanVerification(q.QueryRow(ctx, findByUserSQL, userID.Int64()))
	if errors.Is(err, pgx.ErrNoRows) {
		return Verification{}, ErrVerificationNotFound
	}
	if err != nil {
		return Verification{}, fmt.Errorf("verifyrepo: find by user: %w", err)
	}
	return v, nil
}
...
func (r *Repo) IncrementAttempts(ctx context.Context, q postgres.Querier, userID id.UserID) (int, error) {
	var attempts int
	...
	if err != nil {
		return 0, fmt.Errorf("verifyrepo: increment attempts: %w", err)
	}
	return attempts, nil
}

// ResendParams carries the fields of a re-issued challenge: a fresh PIN hash and
...
// ResendPolicy is the durable resend budget enforced by Resend. A resend is
...
func (r *Repo) Resend(ctx context.Context, q postgres.Querier, params ResendParams, policy ResendPolicy) error {
	...
	if err != nil {
		return fmt.Errorf("verifyrepo: resend: %w", err)
	}
	if updated {
		return nil
	}
...
func (r *Repo) Delete(ctx context.Context, q postgres.Querier, userID id.UserID) error {
	tag, err := q.Exec(ctx, deleteSQL, userID.Int64())
	if err != nil {
		return fmt.Errorf("verifyrepo: delete: %w", err)
	}
