...
func TestEncodeMessage(t *testing.T) {
	got := encodeMessage("general", "alice", "Alice Liddell", "hello world", 1700000000000)
	want := `{"room_id":"general","sender_id":"alice","sender_name":"Alice Liddell","text":"hello world","timestamp":1700000000000,"type":"message"}`
	if string(got) != want {
...
func TestEncodeJoined(t *testing.T) {
	got := encodeJoined("general", "alice", "Alice Liddell")
	want := `{"room_id":"general","type":"joined","user_id":"alice","user_name":"Alice Liddell"}`
	if string(got) != want {
...
func TestEncodeLeft(t *testing.T) {
	got := encodeLeft("general", "alice", "Alice Liddell")
	want := `{"room_id":"general","type":"left","user_id":"alice","user_name":"Alice Liddell"}`
	if string(got) != want {
...
func TestNonAuthErrorBuildersOmitCode(t *testing.T) {
	cases := map[string][]byte{
		...
		"message": encodeMessage("r", "s", "sn", "t", 1),
		"joined":  encodeJoined("r", "u", "un"),
		"left":    encodeLeft("r", "u", "un"),
	}
...
func TestEncodersNeverLeakSensitiveSubstrings(t *testing.T) {
	...
	out1 := encodeMessage("r", "s", "sn", "harmless body", 1)
