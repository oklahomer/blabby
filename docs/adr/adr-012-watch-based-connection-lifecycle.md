...
- The death-watch notification reaches the grain as `middleware.WatchedTerminated`, handled in `ReceiveDefault`; the handler removes the matching entry from the map. No client RPC is involved. The translation is load-bearing: `*actor.Terminated` implements `actor.SystemMessage`, and the protoc-gen-go-grain dispatch discards every `SystemMessage` before its `ReceiveDefault` arm — so the User kind installs `middleware.TranslateTerminated`, a receiver middleware that re-wraps the notification into a plain struct the generated switch routes through.
...
- **Cleanup correctness depends on protoactor's death-watch propagating across the cluster.** `EndpointWatcher` is the supported mechanism, and the integration tests (`TestUserGrain_SenderPID/WatchEvictsOnTermination` and `…/WatchOnDeadPID`) exercise eviction end-to-end through a real cluster, but the path is no longer entirely under our control. A bug in protoactor's cross-node watch would cause silent over-retention. Mitigation: the tests count dead letters at the evicted PID, so a regression anywhere on the notification path — including the generated dispatch swallowing it — fails CI rather than degrading silently.
...
- `internal/grain/user/sender_pid_test.go` — `TestUserGrain_SenderPID/WatchEvictsOnTermination` exercises eviction end-to-end through a real cluster: poison a registered actor, then assert a fan-out round completes without a dead letter at the poisoned PID. `…/WatchOnDeadPID` covers the other arrival order: registering an already-stopped PID, whose watch fires an immediate not-found notification.
